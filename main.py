...
from pla.data.data_utils import (
    flatten_all_map_mmo_results,
    flatten_map_mmo_results,
    flatten_normal_outbreaks,
    flatten_multi,
)
...
def read_mmos():
    ...
    return {
        "results": flatten_all_map_mmo_results(
            results, config.get("FILTER_ON_SERVER", False)
        )
    }
...
def read_one_map():
    results = pla.get_map_mmos(
        reader, request.json["mapname"], request.json["research"], False
    )
    return {
...
def read_normals():
    ...
    return {
        "results": flatten_normal_outbreaks(
            results, config.get("FILTER_ON_SERVER", False)
        )
    }
...
def read_distortions():
    results = pla.check_all_distortions(
        reader, request.json["map_name"], request.json["research"]
    )
...
def check_multispawner():
    results = pla.check_multi_spawner(
        reader,
        request.json["research"],
