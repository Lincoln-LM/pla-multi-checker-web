...
class NXReader(object):
    ...
    def recv(self, size, force=None, timeout=0):
        ...
        if not self.usb_connection:
            ...
            size = int(
                struct.unpack("<L", self.global_in.read(4, timeout=timeout).tobytes())[
                    0
                ]
            )
            if size > 4080:
    ...
    def clear_reads(self):
        try:
            size = int(
                struct.unpack("<L", self.global_in.read(4, timeout=5).tobytes())[0]
            )
    ...
    def write_int(self, address, data, size):
        data = f"{int.from_bytes(int.to_bytes(data,size,'big'),'little'):X}".zfill(
            size * 2
        )
    ...
    def write_pointer(self, pointer, data):
        jumps = pointer.replace("[", "").replace("main", "").split("]")
        self.sendCommand(
            f'pointerPoke 0x{data} 0x{" 0x".join(jump.replace("+","") for jump in jumps)}'
        )
...
class SWSHReader(NXReader):
    ...
    def __init__(self, ip=None, port=6000, usb_connection=False):
        ...
        if self.TrainerSave.isPokemonSave():
            ...
            print(
                f"Game: {self.TrainerSave.GameVersion()}    OT: {self.TrainerSave.OT()}    IDs: {self.TrainerSave.displayID()}|{self.TID:05d}/{self.SID:05d}\n"
            )
...
class LGPEReader(NXReader):
    ...
    def __init__(self, ip=None, port=6000, usb_connection=False):
        ...
        print(
            f"OT: {self.TrainerSave.OT()}    ID: {str(self.TrainerSave.displayID()).zfill(6)}    TID: {str(self.TrainerSave.TID()).zfill(5)}    SID: {str(self.TrainerSave.SID()).zfill(5)}\n"
        )
        self.TID = self.TrainerSave.TID()
        self.SID = self.TrainerSave.SID()
...
class BDSPReader(NXReader):
    ...
    def __init__(self, ip=None, port=6000, usb_connection=False):
        ...
        print(
            f"TID: {self.TrainerSave.TID()}    SID: {self.TrainerSave.SID()}    ID: {self.TrainerSave.displayID()}\n"
        )
        self.TID = self.TrainerSave.TID()
        self.SID = self.TrainerSave.SID()
    ...
    def readWild(self, index=0):
        return self.read_pointer(
            f"[[[[[[[[[[[[[[main+4E853F0]+18]+C0]+28]+B8]]+7E8]+58]+28]+10]+{(0x20+8*index):X}]+20]+18]+20",
            self.PK8bPARTYSIZE,
        )

    def readParty(self, index):
        return self.read_pointer(
            f"[[[[[[[[[[[[[[main+4E853F0]+18]+C0]+28]+B8]]+7F0]+10]+{(0x20+8*index):X}]+20]+18]+20",
            self.PK8bPARTYSIZE,
        )
    ...
    def readGroupSeed(self, index):
        return self.read_pointer(
            f"[[[[[[[main+4E853F0]+18]+C0]+28]+B8]]+348]+{((index * 0x38) + 0x40):X}", 8
        )
