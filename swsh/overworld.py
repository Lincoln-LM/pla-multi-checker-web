...
def autofill(weather, location, type, version, species):
    ...
    if type != "Static":
        ...
        min_level = max_level = encounters[version][type][location][weather][species][
            "Level"
        ]
        forced_ability_var = (
            -1
            if encounters[version][type][location][weather][species]["Ability"] == -1
            else encounters[version][type][location][weather][species]["Ability"]
        )

    if type != "Static":
        min_slot, max_slot = encounters[version][type][location][weather]["Slots"][
            species
        ]
        flawless_ivs = 0
    else:
        flawless_ivs = encounters[version][type][location][weather][species][
            "GuaranteedIVs"
        ]
...
def get_sprite(info, result: OverworldState):
    ...
    if result.is_static:
        ...
        specnames = encounters[info["version"]][info["type"]][info["loc"]][
            info["weather"]
        ]["Slots"]
...
def check_overworld_seed(states, filter: Filter, owoptions, initadv, maxadv, info):
    ...
    predict = OverworldRNG(
        ...
        cute_charm=None
        if owoptions["cute_charm"] == "None"
        else int(owoptions["cute_charm"]),
    ...
    for i in range(0, maxadv - initadv):
        ...
        if result is not None:
            ...
            res[i] = {
                ...
                "species": species,
                "eggmove": "None"
                if result.egg_move is None
                or len(personal.get(species, None).get("Egg_Moves", None))
                < result.egg_move
                else personal.get(species, "None").get("Egg_Moves", "None")[
                    result.egg_move
                ],
            }
