...
def populate_routes(gamever, enctype):
    ...
    encounters = json.load(
        open(RESOURCE_PATH + f"resources/gen3/{gamever}_encounter.json")
    )
...
def check_wilds(
    ...
    genc = json.load(
        open(RESOURCE_PATH + f"resources/gen3/{encounter['version']}_encounter.json")
    )
    table = None
    ...
    for i in range(0, filter["maxadv"] + 1):
        ...
        while (natures(pid % 25) != nature) or (
            cuteCharmFlag and not calcCharm(leadopt, pid)
        ):
            testnature = natures(pid % 25)
            testcalccharm = calcCharm(leadopt, pid)
            low = rng.next_u16()
