...
initchain = [
    "<span class='pla-results-init'>Initial Spawn 4 </span></span>",
    "<span class='pla-results-init'>Initial Spawn 3 </span></span>",
    "<span class='pla-results-init'>Initial Spawn 2 </span></span>",
    "<span class='pla-results-init'>Initial Spawn 1 </span></span>",
]


def generate_mmo_aggressive_path(
    group_seed,
    research,
    paths,
    max_spawns,
    ...
    for init_spawn in range(1, 5):
        ...
        if fixed_seed not in uniques:
            ...
            rolls = (
                ...
                else get_rolls(pokemon, research, BASE_ROLLS_MMOS)
            )
            if isnormal:
                rolls = rolls + 13
            ec, pid, ivs, ability, gender, nature, shiny, square = generate_from_seed(
                fixed_seed, rolls, guaranteed_ivs, fixed_gender
            )
        ...
        for step_num, step in enumerate(path):
            for spawn_on_step in range(1, step + 1):
                ...
                if fixed_seed not in uniques:
                    uniques.add(fixed_seed)
                    path_id = get_path_id(
                        fixed_seed, path[:step_num] + [spawn_on_step], path_num, path
                    )
                    ...
                    storage[path_id] = {
                        ...
                        "chains": [],
                        "defaultroute": not isbonus
                        and sum(path[:step_num]) == len(path[:step_num])
                        and spawn_on_step == 1,
                        "multi": False,
                    }
                else:
...
def get_path_string(steps, final=None):
    return "Path: " + ", ".join(
        f"D{step}" for step in (steps if final is None else steps + [final])
    )
...
def get_map_name(reader, map_index):
    map_id = reader.read_pointer_int(
        f"[[[[[[main+42BA6B0]+2B0]+58]+18]+" f"{0x1d4 + 0xb80*map_index - 0x24:X}", 2
    )
...
def get_mmo_info(reader, map_index, group_id):
    data = reader.read_pointer(
        f"[[[[[[main+42BA6B0]+2B0]+58]+18]+{0x1c0 + 0xb80*map_index + 0x90*group_id:X}",
        0x78,
    )
    return {
        "map_index": map_index,
...
def get_seed_spawned(reader, map_index, group_id):
    data = reader.read_pointer(
        f"[[[[[[main+42BA6B0]+2B0]+58]+18]+{0x218 + 0xb80*map_index + 0x90*group_id:X}",
        0x10,
    )
    return {
...
def get_encounter_pointer(reader, map_index, group_id, bonus):
    ...
    enc_pointer = reader.read_pointer_int(
        f"[[[[[[main+42BA6B0]+2B0]+58]+18]+"
        f"{0x1d4 + 0xb80*map_index + 0x90*group_id + offset:X}",
        8,
    )
...
def get_group_seed(reader, map_index, group_id):
    return reader.read_pointer_int(
        f"[[[[[[main+42BA6B0]+2B0]+58]+18]+"
        f"{0x1d4 + 0xb80*map_index + 0x90*group_id + 0x44:X}",
        8,
    )


def get_gen_seed_to_group_seed(reader, group_id):
    gen_seed = reader.read_pointer_int(
        f"[[[[[[main+42EEEE8]+78]+" f"{0xD48 + 0x8*group_id:X}]+58]+38]+478]+20", 8
    )
...
def get_bonus_flag(reader, map_index, group_id):
    return (
        reader.read_pointer_int(
            f"[[[[[[main+42BA6B0]+2B0]+58]+18]+"
            f"{0x1d4 + 0xb80*map_index + 0x90*group_id + 0x18:X}",
            1,
        )
        == 1
    )
...
def get_group_coordinates(reader, map_index, group_id):
    ...
    coords = struct.unpack(
        ...
        reader.read_pointer(
            f"[[[[[[main+42BA6B0]+2B0]+58]+18]+"
            f"{0x1d4 + 0xb80*map_index + 0x90*group_id - 0x14:X}",
            12,
        ),
    )
...
def get_num_spawned(reader, map_index, group_id):
    return reader.read_pointer_int(
        f"[[[[[[main+42BA6B0]+2B0]+58]+18]+{0x1d4 + 0xb80*map_index + 0x90*group_id + 0x64:X}",
        4,
    )
...
def format_firstround_result(result, group_id, map_name, coords, max_spawns, chained):
    ...
    if result["shiny"]:
        chained[
            result["index"]
        ] = f"<span class='pla-results-firstpath'>First Round </span>{result['index']}"
        print(f"Chained: {chained}")


def format_bonusround_result(
    result, path, epath, group_id, map_name, coords, true_spawns, max_spawns, chained
):
    ...
    if result["shiny"]:
        ...
        frchainstring = (
            frchainstring[: frchainstring.rfind("</span>") - 1]
            .replace("[", "")
            .replace("]", "")
            .replace(", ", "")
            .split("D")[1:]
        ...
        for chain in chained:
            if "+" not in chain and "Initial" not in chain:
                ...
                if len(frpath) <= len(frbonuspath):
                    ...
                    if match:
                        if (
                            len(frpath) == len(frbonuspath)
                            and frpath[len(frpath) - 1]
                            == frbonuspath[len(frbonuspath) - 1]
                        ):
                            print("Exact first round path, adding")
                            result["chains"].append(chained[chain])
                        elif frbonuspath[len(frpath) - 1] >= frpath[len(frpath) - 1]:
                            difference = (
                                frbonuspath[len(frpath) - 1] - frpath[len(frpath) - 1]
                            )
        ...
        for r, res in enumerate(chainresult):
            ...
            if res is not None:
                ...
                if (
                    "Initial" in bonuscheck[len(bonuscheck) - 1]
                    or "FirstRound" in bonuscheck[0]
                    or (
                        len(bonuscheck) < len(currcheck)
                        and bonuscheck[0] == currcheck[0]
                    )
                ):
                    print(
                        "Either an initial spawn, or bonuscheck < currcheck and they're on the same path, adding."
                    )
                    ...
                    len(bonuscheck) == len(currcheck) and bonuscheck[0] == currcheck[0]
                ):
                    bonuscheck = list(map(int, bonuscheck))
                    currcheck = list(map(int, currcheck))
...
def get_bonusround_path_display(index, path, epath):
    ...
    epath_string = (
        "[Clear Round]"
        if epath == []
        else f"<span class='pla-results-revisit'> Revisit {epath} </span>"
    )
    return (
...
def get_bonusround(
    ...
    for path_num, path in enumerate(paths):
        ...
        for ext_num, epath in enumerate(extrapaths):
            ...
            if epath == []:
                results = generate_mmo_aggressive_path(
                    seed,
                    research,
                    nbpaths,
                    bonus_spawns,
...
def get_all_map_mmos(reader, research, inmap, rolls_override=None):
    ...
    for map_index in range(MAX_MAPS):
        ...
        if map_name is not None:
            ...
            results[map_name] = get_map_mmos(
                reader, map_index, research, inmap, rolls_override
            )
...
def get_map_mmos(
    reader, map_index, research, inmap, rolls_override=None, map_name=None
):
    ...
    for group_id in range(MAX_MMOS):
        firstround, bonusround = get_mmo(
            reader, map_index, group_id, research, inmap, rolls_override, map_name
        )
...
def get_mmo(
    reader, map_index, group_id, research, inmap, rolls_override=None, map_name=None
):
    mmoinfo = get_mmo_info(reader, map_index, group_id)
    ...
    if mmoinfo["species_index"] == 0:
        return None, None
    print(
        f"Species Group: {pokedex.entry_by_index(mmoinfo['species_index']).display_name()}"
    )
    ...
    group_seed = (
        get_gen_seed_to_group_seed(reader, group_id) if inmap else mmoinfo["group_seed"]
    )
    return mmo_from_seed(
        group_id,
        research,
        group_seed,
        map_name,
        mmoinfo["coords"],
        mmoinfo["fr_encounter"],
        mmoinfo["br_encounter"],
        has_bonus,
        mmoinfo["fr_spawns"],
        mmoinfo["br_spawns"],
        rolls_override,
...
def mmo_from_seed(
    ...
    firstround = generate_mmo_aggressive_path(
        group_seed,
        research,
        paths,
        max_spawns,
    ...
    for result in firstround.values():
        format_firstround_result(
            result, group_id, map_name, coords, max_spawns, chained
        )
    ...
    if has_bonus:
        ...
        print(
            f"Bonus Round Species: {'Alpha ' if alpha else ''}{pokemon.display_name()}"
        )

        bonusround = get_bonusround(
            bonusround_paths,
            group_id,
            research,
            group_seed,
...
def check_mmo_from_seed(
    ...
    outbreaks = {}
    firstround, bonusround = mmo_from_seed(
        group_id,
        research,
        group_seed,
        map_name,
