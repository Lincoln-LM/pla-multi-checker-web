...
class OverworldState:
    ...
    def row(self):
        ...
        row += [
            f"{self.ec:08X}",
            f"{self.pid:08X}",
            "No" if self.xor >= 16 else ("Square" if self.xor == 0 else "Star"),
            self.natures[self.nature],
        ]
        if not self.hide_ability:
            row += [self.ability]
        row += [
            self.genders[self.gender],
            "/".join(str(iv) for iv in self.ivs),
            self.mark,
            self.height,
            self.weight,
        ]
...
class Filter:
    ...
    def __init__(
        ...
        self.shininess = (
            Filter.shiny_list.index(shininess) if shininess != None else None
        )
        self.slot_min = slot_min
        self.slot_max = slot_max
        self.natures = (
            [Filter.nature_list.index(nature) for nature in natures]
            if natures != None
            else None
        )
        self.marks = marks
    ...
    def compare_fixed(self, state: OverworldState):
        ...
        if not self.weight_min == None and not (
            self.weight_min <= state.weight <= self.weight_max
            and self.height_min <= state.height <= self.height_max
        ):
            return False
    ...
    def compare_slot(self, state: OverworldState):
        return (
            self.slot_min <= state.slot_rand <= self.slot_max
            if self.slot_min != None
            else True
        )
...
class OverworldRNG:
    ...
    def __init__(
        ...
        (
            self.brilliant_thresh,
            self.brilliant_rolls,
        ) = OverworldRNG.calculate_brilliant_info(kos)
    ...
    def generate_filter(self):
        ...
        if self.is_static:
            ...
            if self.min_level != self.max_level:
                state.level = self.min_level + go.rand(
                    self.max_level - self.min_level + 1
                )
            else:
                state.level = self.min_level
            state.mark = OverworldRNG.rand_mark(
                go, self.weather_active, self.is_fishing, self.mark_charm
            )
        ...
        if not self.is_shiny_locked:
            for roll in range(
                (3 if self.shiny_charm else 1)
                + (self.brilliant_rolls if state.brilliant else 0)
            ):
        ...
        (
            ...
            state.fixed_seed, self.tsv, shiny, self.flawless_ivs + brilliant_ivs
        )
    ...
    def rand_mark(go, weather_active, is_fishing, mark_charm):
        for roll in range(3 if mark_charm else 1):
            ...
            if personality_rand == 0:
                return OverworldRNG.personality_marks[
                    go.rand(len(OverworldRNG.personality_marks))
                ]
            if uncommon_rand == 0:
                return "Uncommon"
            if weather_rand == 0:
    ...
    def calculateFromPKM(pkm):
        return OverworldRNG.calculate_fixed(
            pkm.seed, pkm.tid ^ pkm.sid, pkm.setShininess != 3, pkm.setIVs
        )[:3]
...
class Egg:
    ...
    def __init__(self, seed, parent1, parent2, shinycharm, tid=0, sid=0):
        ...
        self.InheritIVsCnt = (
            5
            if Male.helditem == Egg.DESTINYKNOT or Female.helditem == Egg.DESTINYKNOT
            else 3
        )
