...
def read_wild_rng(map_name, index, group_seed, research, rolls_override=None):
    ...
    rolls = (
        ...
        else get_rolls(pokemon, research, BASE_ROLLS)
    )
    fixed_seed = rng.next()

    ec, pid, ivs, ability, gender, nature, shiny, square = generate_from_seed(
        fixed_seed, rolls, 3 if alpha else 0, pokemon.is_fixed_gender()
    )
...
def get_generator_seed(reader, map_name, distortion_index):
    return reader.read_pointer_int(
        f"[[[[[[main+42CC4D8]+C0]+1C0]+{distortion_offset[map_name] + distortion_index * 0x8:X}]+18]+430]+C0",
        8,
    )
...
def get_distortion_spawns(map_name):
    return [
        s["species"] for s in distortions[map_name]["encounters"] if s["alpha"] == False
    ]
...
def get_distortion_locations(map_name):
    return [
        loc
        for loc in distortions[map_name]["locations"].values()
...
def get_encounter_slot_max(map_name, distortion_index):
    if (
        map_name == "crimsonmirelands"
        and distortion_index >= 13
        and distortion_index < 16
    ):
...
def is_common_spawn(distortion_index, distortion_name):
    return (
        distortion_index in [0, 4, 8, 12, 16, 20]
        or distortion_name.lower() == "unknown"
    )
...
def check_all_distortions(reader, map_name, research, rolls_override=None):
    return list(
        filter(
            None,
            (
                check_distortion(reader, map_name, i, research, rolls_override)
                for i in range(distortions[map_name]["number"])
            ),
        )
    )
