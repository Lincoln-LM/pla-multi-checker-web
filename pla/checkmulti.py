...
def generate_spawn2(rng, encounters, encsum, path):
    ...
    ec, pid, ivs, ability, gender, nature, shiny, square = generate_from_seed(
        fixed_seed, enc["rolls"], enc["guaranteed_ivs"], enc["fixed_gender"]
    )
...
def check_multi_spawner(
    reader, research, group_id, maxspawns, maxdepth, isnight, rolls_override=None
):
    ...
    return check_multi_spawner_seed(
        group_seed, research, group_id, maxspawns, maxdepth, isnight, rolls_override
    )


def check_multi_spawner_seed(
    group_seed, research, group_id, maxspawns, maxdepth, isnight, rolls_override=None
):
    if maxdepth > 18:
        maxdepth = 5
