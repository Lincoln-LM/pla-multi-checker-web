...
SAVE_KEYS = {"pokedex": 0x02168706, "key_items": 0x59A4D0C3}
...
def read_block(data, offset):
    ...
    if key in SAVE_KEYS.values():
        ...
        num_bytes = (
            int.from_bytes(data[offset : offset + 4], byteorder="little") ^ rng.next32()
        )
...
def read_pokedex(blockdata):
    if len(blockdata) != POKEDEX_BLOCK_SIZE:
        return None, {
            "error": "There was a problem checking your pokedex research levels"
        }

    return [
        read_research_entry(blockdata, 0x70 + (0x58 * i)) for i in range(MAX_SPECIES)
    ], None


def read_research_entry(data, offset):
    flags = int.from_bytes(data[offset : offset + 4], byteorder="little")
    research_rate = int.from_bytes(
        data[offset + 0x08 : offset + 0x08 + 2], byteorder="little"
    )
    return {"complete": research_rate >= 100, "perfect": flags & (1 << 2) != 0}
...
def has_shinycharm(blockdata):
    ...
    if len(blockdata) < 400:
        return None, {
            "error": "There was a problem checking if you have the shiny charm"
        }
