...
def get_all_outbreaks(reader, research, inmap, rolls_override=None):
    ...
    for map_index in range(MAX_MAPS):
        pokemon, group_seed, max_spawns, coordinates = get_outbreak_info(
            reader, map_index, inmap
        )
        ...
        if pokemon is not None:
            rolls = (
                ...
                else get_rolls(pokemon, research, BASE_ROLLS_OUTBREAKS)
            )
            ...
            for result in results.values():
                format_outbreak_result(
                    result, pokemon, map_index, max_spawns, coordinates
                )
...
def generate_outbreak_aggressive_path(
    group_seed, rolls, steps, uniques, paths, storage
):
    ...
    for init_spawn in range(1, 5):
        ...
        if not fixed_seed in uniques:
            ...
            ec, pid, ivs, ability, gender, nature, shiny, square = generate_from_seed(
                fixed_seed, rolls, 3 if alpha else 0
            )
        ...
        for pokemon in range(1, step + 1):
            ...
            if not fixed_seed in uniques:
                ...
                storage[str(fixed_seed)] = {
                    ...
                    "gender": gender,
                    "rolls": rolls,
                    "defaultroute": len(steps[:step_i]) == sum(steps[:step_i])
                    and pokemon == 1,
                }
...
def pathfind_aggressive_outbreak(
    group_seed,
    rolls,
    spawns,
    step=0,
    ...
    if sum(_steps) + step < spawns - 4:
        for _step in range(1, min(5, (spawns - 4) - sum(_steps))):
            if (
                pathfind_aggressive_outbreak(
                    group_seed, rolls, spawns, _step, _steps, uniques, paths, storage
                )
                is not None
            ):
        ...
        generate_outbreak_aggressive_path(
            group_seed, rolls, _steps, uniques, paths, storage
        )
...
def get_outbreak_info(reader, group_id, inmap):
    species_index = reader.read_pointer_int(
        f"[[[[[[main+42BA6B0]+2B0]+58]+18]+" f"{0x20 + 0x50*group_id:X}", 2
    )
    if species_index == 0:
        return None, 0, 0, None

    group_seed = (
        get_gen_seed_to_group_seed(reader, group_id)
        if inmap
        else get_group_seed(reader, group_id, 0)
    )

    max_spawns = reader.read_pointer_int(
        f"[[[[main+42BA6B0]+2B0]+58]+18]+" f"{0x20 + 0x50*group_id + 0x40:X}", 8
    )

    coords = struct.unpack(
        "fff",
        reader.read_pointer(
            f"[[[[[[main+42BA6B0]+2B0]+58]+18]+" f"{0x20 + 0x50*group_id + 0x20:X}", 12
        ),
    )
