...
if config["SeedCheckOnly"]:
    print(
        "Seed Check only mode! Note: You will not be able to use MMO checker or Distiortion Checker!"
    )
...
def ug():
    return render_template(
        "pages/underground.html", title="Underground Checker", bdsp="true"
    )
...
def bdsp_stationary():
    return render_template(
        "pages/b_stationary.html", title="Stationary Checker", bdsp="true"
    )
...
def owrng():
    return render_template(
        "pages/overworld.html", title="Overworld Checker", swsh="true"
    )
...
def g3static():
    return render_template(
        "pages/rse_static.html", title="RSE Static Checker", g3="true"
    )
...
def check_gen3_static():
    ...
    return {
        "results": flatten_bdsp_stationary(
            results, config.get("FILTER_ON_SERVER", False), filter_command
        )
    }
...
def g3_pop_species():
    results = gen3.populate_species(
        request.json["version"], request.json["type"], request.json["location"]
    )
...
def get_from_seed():
    ...
    results = pla.check_mmo_from_seed(
        group_seed,
    ...
    return {
        "results": flatten_map_mmo_results(
            results, config.get("FILTER_ON_SERVER", False), filter_command
        )
    }
...
def get_alpha_from_seed():
    ...
    results = pla.check_alpha_from_seed(
        group_seed,
        request.json["rolls"],
...
def check_multiseed():
    ...
    results = pla.check_multi_spawner_seed(
        group_seed,
        request.json["research"],
...
def read_savefile():
    if request.method == "POST":
        ...
        if save:
            ...
            rolls = {
                pkm.species: rolls_from_research(
                    results["research_entries"], pkm.dex_number()
                )
                for pkm in hisuidex
            }

            return {"shinycharm": results["shinycharm"], "rolls": rolls}
...
def check_bdsp_stationary():
    ...
    results = bdsp.read_stationary_seed(
        states,
        request.json["filter"],
...
def check_bdsp_egg():
    ...
    results = bdsp.read_egg_seed(
        states, request.json["filter"], request.json["daycare"], request.json["delay"]
    )
...
def check_bdsp_tid():
    ...
    results = bdsp.read_tid_seed(states, request.json["filter"], request.json["ids"])
...
def pop_weather():
    results = swsh.populate_weather(
        request.json["loc"], request.json["type"], request.json["version"]
    )
...
def pop_species():
    results = swsh.populate_species(
        request.json["weather"],
        request.json["loc"],
        request.json["type"],
        request.json["version"],
    )
...
def check_ow():
    ...
    filters = Filter(
        request.json["filter"]["minivs"],
        request.json["filter"]["maxivs"],
        None,
        request.json["filter"]["shiny_filter"]
        if request.json["filter"]["shiny_filter"] != "None"
        else None,
        request.json["filter"]["slot_min"],
        request.json["filter"]["slot_max"],
        None,
        None,
        request.json["filter"]["brilliant"],
        None,
        None,
        None,
        None,
    ...
    results = swsh.check_overworld_seed(
        states,
        filters,
        request.json["options"],
        request.json["initadv"],
        request.json["maxadv"],
        request.json["info"],
    )
